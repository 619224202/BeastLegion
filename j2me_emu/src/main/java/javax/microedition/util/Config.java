...
public class Config {
	...
	private static int origon_ScreenW = 1280;
	private static int origon_ScreenH = 720;

	private static Activity startActivity = null;
	...
	public static InputStream loadResources(String file){
		if(startActivity==null){
			System.out.println("获取资源文件的容器为 空，请设置");
			return null;
		}
		InputStream is = null;
		try {
			is = startActivity.getAssets().open(file);
		} catch (IOException e) {
			e.printStackTrace();
		}
		return is;
	...
	public static void setOrigonWH(int width,int height){
		origon_ScreenW = width;
		origon_ScreenH = height;
	}

	public static int getOrigon_ScreenW() {
		return origon_ScreenW;
	}

	public static int getOrigon_ScreenH() {
		return origon_ScreenH;
	}
}
