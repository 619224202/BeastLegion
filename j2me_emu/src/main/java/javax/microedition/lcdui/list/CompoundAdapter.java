...
public abstract class CompoundAdapter implements Adapter {
	...
	protected View getView(int position, View convertView, ViewGroup parent, int viewResourceID, boolean useImagePart) {
		...
		final CompoundItem item = items.get(position);
		...
		if (useImagePart) {
			View.OnLayoutChangeListener layoutChangeListener = new View.OnLayoutChangeListener() {
				...
				public void onLayoutChange(View v, int left, int top, int right, int bottom, int oldLeft, int oldTop, int oldRight, int oldBottom) {
					fTextView.setCompoundDrawablesWithIntrinsicBounds(item.getDrawable(fTextView.getHeight()),
							null, null, null);
					fTextView.setCompoundDrawablePadding(fTextView.getPaddingLeft());
				}
			};
